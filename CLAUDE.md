...
│       ├── IOSControlClient/ # AutomationServer HTTP 클라이언트 + simctl 래퍼
│       └── Playground/      # IOSControlClient 테스트용 실행 파일
├── AutomationServer/    # iOS 자동화 서버 (Xcode 프로젝트)
│   └── AutomationServerTests/ # XCTest 번들 (HTTP 서버 + UI 조작)
...
- `make agent`: AutomationServer 빌드
- `make agent-run`: AutomationServer 빌드 및 실행
...
iOS 시뮬레이터에서 일반 앱은 네트워크 바인딩이 제한되지만, XCTest는 특별한 권한을 가짐. AutomationServer는 XCTest 번들로 구성되어 이 권한을 활용.
...
- AutomationServer는 `SIMULATOR_UDID` 환경변수로 자신이 실행 중인 시뮬레이터 UDID를 알 수 있음.
