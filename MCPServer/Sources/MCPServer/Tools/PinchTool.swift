...
struct PinchTool: MCPTool {
    ...
    static let inputSchema: Value = .object([
        "type": .string("object"),
        "properties": .object([
            "center": .object(["type": .string("string"), "description": .string("Pinch center coordinate as 'x,y' (e.g., '200,400'). Default is screen center")]),
    ...
    static func execute(args: PinchArgs, client: any AgentClient) async throws -> [Tool.Content] {
        ...
        let centerCoord = args.parseCenter()
        let x = centerCoord?.x ?? (frame.width / 2)
        let y = centerCoord?.y ?? (frame.height / 2)
